...
void DataEnc::setLength(mlong length) {
    putLong(length, 8);
}
...
DataEnc &DataEnc::putInt(int val) {
    if ((index + 4) <= m_byteLen) {
        ByteUtil::intToBytes(val, m_bytes, index);
        index += 4;
    }
    return *this;
}
...
DataEnc &DataEnc::putLong(mlong val) {
    if ((index + 8) <= m_byteLen) {
        ByteUtil::longToBytes(val, m_bytes, index);
        index += 8;
    }
    return *this;
}
...
DataEnc &DataEnc::putStr(const char *str, int len) {
    if ((index + len + 4) <= m_byteLen) {
        putInt(len);
        ByteUtil::ByteArrCopy((mbyte *) str, 0, m_bytes, index, len);
        index += len;
    }
    return *this;
}
...
DataEnc &DataEnc::putInt(int val, int i) {
    if ((i + 4) <= m_byteLen) {
        ByteUtil::intToBytes(val, m_bytes, i);
    }
    return *this;
}
...
DataEnc &DataEnc::putLong(mlong val, int i) {
    if ((i + 8) <= m_byteLen) {
        ByteUtil::longToBytes(val, m_bytes, i);
    }
    return *this;
}
...
DataEnc &DataEnc::putStr(const char *str, int len, int i) {
    if ((i + len + 4) <= m_byteLen) {
        putInt(len, i);
        i += 4;
        ByteUtil::ByteArrCopy((mbyte *) str, 0, m_bytes, i, len);
    }
    return *this;
}
