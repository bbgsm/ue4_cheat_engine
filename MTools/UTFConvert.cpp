...
string UTFConvert::utf16_to_utf8(const u16string &u16str) {
...
// 从UTF16BE编码字符串创建
string utf16be_to_utf8(const u16string &u16str) {
    if (u16str.empty()) { return string(); }
    const char16_t *p = u16str.data();
    u16string::size_type len = u16str.length();
    if (p[0] == 0xFEFF) {
...
u16string utf8_to_utf16le(const string &u8str, bool addbom, bool *ok) {
    u16string u16str;
...
// 获取转换为UTF-16 BE的字符串
u16string utf8_to_utf16be(const string &u8str, bool addbom, bool *ok) {
    // 先获取utf16le编码字符串
    u16string u16str = utf8_to_utf16le(u8str, addbom, ok);
    // 将小端序转换为大端序
